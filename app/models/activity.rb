class Activity < ActiveRecord::Base
  ...
  scope :for_all_users, -> { where(is_user_only: false) }

  scope :discussions, -> { where('comments_count > 0') }
  scope :changes, -> { where.not(change_id: nil) }
  scope :points, -> { where('type like ?', 'ActivityPoint%').order(created_at: :desc) }
  scope :points_and_docs, -> { where('type like ? or type like ?', 'ActivityPoint%', 'ActivityDocument%').order(created_at: :desc) }
  scope :capital, -> { where('type like ?', '%Capital%') }
  scope :interesting, -> { where("type in ('ActivityPriorityMergeProposal','ActivityPriorityAcquisitionProposal') or comments_count > 0") }
  
  scope :last_three_days, -> { where('changed_at > ?', Time.now - 3.days) }
  scope :last_seven_days, -> { where('changed_at > ?', Time.now - 7.days) }
  scope :last_thirty_days, -> { where('changed_at > ?', Time.now - 30.days) }
  scope :last_24_hours, -> { where('created_at > ?', Time.now - 24.hours) }  
  
  scope :by_recently_updated, -> { order(changed_at: :desc) }
